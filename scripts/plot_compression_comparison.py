...
def plot_compression_comparison():
    ...
    ax1.set_ylabel('Compression Ratio', fontsize=16)
    ax1.set_title('(a) Compression Efficiency', fontsize=16)
    ax1.set_xticks(x_pos)
    ax1.set_xticklabels(data_types, rotation=45, fontsize=14)
    ...
    for i, (sf, ss, cxl) in enumerate(zip(scaleflux_ratios, samsung_ratios, cxl_ratios)):
        if sf > 3.0:  # Only label high ratios
            ax1.text(i - width, sf + 0.1, f'{sf:.1f}×', ha='center', fontsize=12)
        if ss > 3.0:
            ax1.text(i, ss + 0.1, f'{ss:.1f}×', ha='center', fontsize=12)
        if cxl > 3.0:
            ax1.text(i + width, cxl + 0.1, f'{cxl:.1f}×', ha='center', fontsize=12)
    ...
    ax2.set_xlabel('Data Type', fontsize=16)
    ax2.set_ylabel('Throughput (% of baseline)', fontsize=16)
    ax2.set_title('(b) Compression Overhead', fontsize=16)
    ax2.set_xticks(x_pos)
    ax2.set_xticklabels(data_types, rotation=45, fontsize=14)
    ...
    ax2.text(3, 101, 'No compression baseline', fontsize=12, alpha=0.7)
